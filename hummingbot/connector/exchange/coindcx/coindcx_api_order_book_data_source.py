...
from . import coindcx_utils
from .coindcx_active_order_tracker import CoindcxActiveOrderTracker
from .coindcx_order_book import CoindcxOrderBook
from .coindcx_websocket import CoindcxWebsocket
from .coindcx_utils import ms_timestamp_to_s
        ...
        async with aiohttp.ClientSession() as client:
            ...
            for t_pair in trading_pairs:
                last_trade = [o["a"] for o in resp_json["result"]["data"] if o["i"] ==
                              coindcx_utils.convert_to_exchange_trading_pair(t_pair)]
    ...
    async def get_order_book_data(trading_pair: str) -> Dict[str, any]:
        ...
        async with aiohttp.ClientSession() as client:
            orderbook_response = await client.get(
                ...
                f"{coindcx_utils.convert_to_exchange_trading_pair(trading_pair)}"
            )
        ...
        while True:
            try:
                ...
                await ws.subscribe(list(map(
                    lambda pair: f"trade.{coindcx_utils.convert_to_exchange_trading_pair(pair)}",
                    self._trading_pairs
                )))
                ...
                async for response in ws.on_message():
                    ...
                    for trade in response["result"]["data"]:
                        ...
                        trade_msg: OrderBookMessage = CoindcxOrderBook.trade_message_from_exchange(
                            trade,
                            trade_timestamp,
                            metadata={"trading_pair": coindcx_utils.convert_from_exchange_trading_pair(trade["i"])}
                        )
    ...
    async def listen_for_order_book_diffs(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        ...
        while True:
            try:
                ...
                await ws.subscribe(list(map(
                    lambda pair: f"book.{coindcx_utils.convert_to_exchange_trading_pair(pair)}.150",
                    self._trading_pairs
                )))
                ...
                async for response in ws.on_message():
                    ...
                    orderbook_msg: OrderBookMessage = CoindcxOrderBook.snapshot_message_from_exchange(
                        order_book_data,
                        timestamp,
                        metadata={"trading_pair": coindcx_utils.convert_from_exchange_trading_pair(
