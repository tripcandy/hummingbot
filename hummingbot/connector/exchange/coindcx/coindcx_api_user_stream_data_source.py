...
from .coindcx_auth import CoindcxAuth
...
class CoindcxAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    async def _listen_to_orders_trades_balances(self) -> AsyncIterable[Any]:
        ...
        try:
            ws = CoindcxWebsocket(self._coindcx_auth)
            await ws.connect()
        ...
        *required
        ...
        while True:
            try:
                ...
                self.logger().error(
                    "Unexpected error with Coindcx WebSocket connection. " "Retrying after 30 seconds...", exc_info=True
                )
