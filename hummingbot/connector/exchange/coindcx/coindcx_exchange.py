...
class CoindcxExchange(ExchangeBase):
    """
    CoindcxExchange connects with Crypto.com exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
        ...
        """
        :param coindcx_api_key: The API key to connect to private Crypto.com APIs.
        :param coindcx_secret_key: The API secret.
        ...
        self._coindcx_auth = CoindcxAuth(coindcx_api_key, coindcx_secret_key)
        self._order_book_tracker = CoindcxOrderBookTracker(trading_pairs=trading_pairs)
        self._user_stream_tracker = CoindcxUserStreamTracker(self._coindcx_auth, trading_pairs)
        self._ev_loop = asyncio.get_event_loop()
        self._shared_client = None
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._in_flight_orders = {}  # Dict[client_order_id:str, CoindcxInFlightOrder]
    ...
    def _format_trading_rules(self, instruments_info: Dict[str, Any]) -> Dict[str, TradingRule]:
        ...
        for rule in instruments_info["result"]["instruments"]:
            try:
                trading_pair = coindcx_utils.convert_from_exchange_trading_pair(rule["instrument_name"])
    ...
    async def _api_request(self,
        ...
        if is_auth_required:
            request_id = coindcx_utils.RequestId.generate_request_id()
            data = {"params": params}
            params = self._coindcx_auth.generate_auth_dict(path_url, request_id, coindcx_utils.get_ms_timestamp(), data)
            headers = self._coindcx_auth.get_headers()
        else:
    ...
    def buy(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        """
        order_id: str = coindcx_utils.get_new_client_order_id(True, trading_pair)
    ...
    def sell(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        :returns A new internal order id
        """
        order_id: str = coindcx_utils.get_new_client_order_id(False, trading_pair)
    ...
    async def _create_order(self,
        ...
        api_params = {"instrument_name": coindcx_utils.convert_to_exchange_trading_pair(trading_pair),
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = CoindcxInFlightOrder(
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> str:
        ...
        try:
            ...
            await self._api_request(
                ...
                {"instrument_name": coindcx_utils.convert_to_exchange_trading_pair(trading_pair),
    ...
    def _process_order_message(self, order_msg: Dict[str, Any]):
        ...
        if tracked_order.is_cancelled:
            ...
            self.logger().info(f"The market order {client_order_id} has failed according to order status API. "
                               f"Reason: {coindcx_utils.get_api_reason(order_msg['reason'])}")
            self.trigger_event(MarketEvent.OrderFailure,
                               MarketOrderFailureEvent(
                                   self.current_timestamp,
                                   client_order_id,
    ...
    async def cancel_all(self, timeout_seconds: float):
        ...
        tracked_orders: Dict[str, CoindcxInFlightOrder] = self._in_flight_orders.copy().items()
        ...
        try:
            ...
            for _, order in tracked_orders:
                api_params = {
                    "instrument_name": coindcx_utils.convert_to_exchange_trading_pair(order.trading_pair),
    ...
    async def _user_stream_event_listener(self):
        ...
        CoindcxAPIUserStreamDataSource.
        """
    ...
    async def get_open_orders(self) -> List[OpenOrder]:
        ...
        for order in result["result"]["order_list"]:
            if coindcx_utils.HBOT_BROKER_ID not in order["client_oid"]:
                continue
            if order["type"] != "LIMIT":
            ...
            ret_val.append(
                OpenOrder(
                    ...
                    trading_pair=coindcx_utils.convert_from_exchange_trading_pair(order["instrument_name"]),
